...
export default function EmrPage() {
  ...
  const interactions = useMemo(() => {
    ...
    meds.forEach((m) => (INTERACTIONS[m] ?? []).forEach((x) => { if (meds.includes(x.with)) found.push({ a: m, b: x.with, severity: x.severity, note: x.note }); }));
    return found;
  }, [meds]);

  const allergyConflict = useMemo(() => {
    if (!patient?.allergies) return [];
    return meds.filter((m) => patient.allergies!.toLowerCase().includes("penicillin") && m.toLowerCase().includes("amox"));
  }, [meds, patient]);
    ...
    <Shell>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="mint" onClick={() => { setSaved(new Date().toLocaleTimeString()); toast("Consultation saved to EMR"); }}><Save className="h-4 w-4" /> Save Record</Button>
        </div>
      </div>
        ...
        <div className="space-y-5 lg:col-span-2">
          ...
          <div className="card p-5">
            ...
            <div className="relative mb-3">
              ...
              {medSearch && (
                <div className="absolute z-20 mt-1 max-h-64 w-full overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl dark:border-white/10 dark:bg-[#1a2b25]">
                  {ALL_MEDS.filter((m) => m.name.toLowerCase().includes(medSearch.toLowerCase()) && !meds.includes(m.name)).map((m, i) => (
                    <button key={i} onClick={() => { setMeds([...meds, m.name]); setMedSearch(""); }} className="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-mint/10 dark:text-white">
                  ...
                  {ALL_MEDS.filter((m) => m.name.toLowerCase().includes(medSearch.toLowerCase()) && !meds.includes(m.name)).length === 0 && (
              ...
              {MEDICINE_CATALOG.map((cat) => (
                <details key={cat.category} className="group rounded-lg border border-slate-100 dark:border-white/10">
                  ...
                  <div className="space-y-1 px-2 pb-2">
                    {cat.items.map((m) => {
                      const picked = meds.includes(m.name);
                      return (
                        <button
                          key={m.name}
                          disabled={picked}
                          onClick={() => setMeds([...meds, m.name])}
            ...
            <div className="flex flex-wrap gap-2">
              {meds.map((m) => {
                const info = ALL_MEDS.find((x) => x.name === m);
                return (
                  <span key={m} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium" style={{ background: `${info?.color ?? "#7a9e7e"}22`, color: info?.color ?? "#7a9e7e" }}>
                    💊 {m}
                    {info && <span className="text-[10px] opacity-70">({info.dose} {info.freq})</span>}
                    <button onClick={() => setMeds(meds.filter((x) => x !== m))} className="ml-1 opacity-60 hover:opacity-100">✕</button>
                  </span>
                );
              })}
