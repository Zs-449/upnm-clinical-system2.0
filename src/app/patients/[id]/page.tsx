...
interface Vitals { bpS?: number; bpD?: number; hr?: number; temp?: number; spo2?: number; weight?: number; height?: number; }
interface MedicalRecord { id: number; patientId: number; doctorName: string; chiefComplaint: string | null; vitals: Vitals | null; diagnosis: string | null; treatmentPlan: string | null; prescriptionId: number | null; createdAt: string; }
...
function PatientProfile() {
  ...
  const { data: md } = useApi<{ medicalRecords: MedicalRecord[] }>(`/api/medical-records?patientId=${pid}`);
  ...
  const records = md?.medicalRecords ?? [];
        ...
        <div className="p-5">
          {tab === "History" && (
            <div className="space-y-6">
              ...
              {/* Consultation / medical records, persisted from the EMR's Save Record action */}
              <div>
                <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Consultations</h4>
                {records.length ? (
                  <div className="space-y-2">
                    {records.map((r) => (
                      <div key={r.id} className="rounded-xl border border-slate-100 p-3 dark:border-white/10">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <span className="text-sm font-semibold text-navy dark:text-white">{new Date(r.createdAt).toLocaleDateString()} · Dr. {r.doctorName.replace(/^Dr\.\s*/, "")}</span>
                          {r.prescriptionId && <Badge color="mint">Rx #{r.prescriptionId}</Badge>}
                        </div>
                        <div className="mt-2 space-y-1 text-xs text-slate-500 dark:text-slate-400">
                          {r.chiefComplaint && <div><span className="font-semibold text-slate-600 dark:text-slate-300">Chief Complaint:</span> {r.chiefComplaint}</div>}
                          {r.vitals && (
                            <div>
                              <span className="font-semibold text-slate-600 dark:text-slate-300">Vitals:</span>{" "}
                              {[
                                r.vitals.bpS != null && r.vitals.bpD != null ? `BP ${r.vitals.bpS}/${r.vitals.bpD}` : null,
                                r.vitals.hr != null ? `HR ${r.vitals.hr}bpm` : null,
                                r.vitals.temp != null ? `Temp ${r.vitals.temp}°C` : null,
                                r.vitals.spo2 != null ? `SpO₂ ${r.vitals.spo2}%` : null,
                              ].filter(Boolean).join(" · ")}
                            </div>
                          )}
                          {r.diagnosis && <div><span className="font-semibold text-slate-600 dark:text-slate-300">Diagnosis:</span> {r.diagnosis}</div>}
                          {r.treatmentPlan && <div><span className="font-semibold text-slate-600 dark:text-slate-300">Treatment Plan:</span> {r.treatmentPlan}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
